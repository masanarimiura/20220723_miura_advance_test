    ...
    <form action="/" method="POST" class="h-adr">
      ...
      <table>
        ...
        <tr>
          <th>ご意見<span >※</span></th>
          <td colspan="2">
            <input type="text" name="opinion" value="{{ old('opinion') }}" class="opinion" id="opinion" size="200" maxlength="100">
  ...
  window.addEventListener('DOMContentLoaded', function(){
    ...
    postcode.addEventListener("input",function(){
      ...
      if(this.value.length <= 6) {
        ...
        } else if(postcodePattern1.test(this.value) == "false"){
          postcode_error.className = 'error';
        }
      } else if(this.value.length == 8 ){
        if(postcodePattern2.test(this.value)){
          postcode_error.className = 'notError';
        } else if(postcodePattern3.test(this.value)){
          postcode_error.className = 'notError';
        } else{
          postcode_error.className = 'error';
        }
