...
class ContactController extends Controller
{
    ...
    public function input(Request $request)
    {
        ...
        $postcode = mb_convert_kana($postcode, "a");
        if(strlen($postcode) == 7){
            $postcode = substr($postcode ,0,3) . "-" . substr($request->postcode ,3);
            $items = [
            ...
            $request->postcode = mb_convert_kana($request->postcode, "a");
            $postcode = substr($request->postcode ,0,3) . "-" . substr($request->postcode ,4);
            $items = [
                'first_name' =>$request->first_name,
                'last_name' => $request->last_name,
                'gender' => $request->gender,
